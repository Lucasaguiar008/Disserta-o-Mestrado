...
while ~solucao_valida
    ...
    while (~isempty(C) && ~isempty(formigas_disponiveis)) %verifica se todos os wp foram visitados ou se tem formigas(vants) disponíveis
        ...
        % Validação da rota
        if validacao_rota_com_pesos(formiga_i,P,cjk,xijk,distMax_voo,Pesos,capacidadeMaxima)
            ...
            [validacao_form, P, C] = validacao_formiga_com_pesos(formiga_i,P,cjk,xijk,distMax_voo,C,saturou,Pesos,capacidadeMaxima);               
