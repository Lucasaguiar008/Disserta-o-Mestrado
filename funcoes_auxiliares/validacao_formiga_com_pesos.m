...
function [validacao_formiga, P, C] = validacao_formiga_com_pesos(formiga_i,P,cjk,xijk,distMax_voo,C,s_saturou,Pesos,capacidadeMaxima)    
    i =formiga_i;     
    rotas_validas = [];

    %verificando se a formiga atingiu a capacidade máxima de coleta de pesos
    if (sum(Pesos(P{i}))>=capacidadeMaxima)
        ...
        Cp = custoParcial(xijk{i},cjk);  % Custo parcial da rota
        ...
        if isempty(rotas_validas)
            ...
            if s_saturou
                ...
                Ponto_s_novo = Pesos(s_novo); % verificando os pontos das rotas candidatas
                Ponto_s_novo = (Ponto_s_novo<=capacidadeMaxima-sum(Pesos(P{i}))); %verifica as rotas candidatas que tem pontuação válida
                s_novo = s_novo(Ponto_s_novo==1); % retornar apenas as rotas
                
                % pega a rota válida com menor distância
                if length(s_novo)>1                
                ...
                % se não tiver rotas válidas a formiga pode encerrar
                if (isempty(s_novo))
                    validacao_formiga = false;
                else
