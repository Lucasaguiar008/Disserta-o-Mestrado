...
resultado_DWA = mutipleDWA_comRestricaoTempo(Nv,rotas,Obstaculos,"Resultado ACO+A*+DWA",numfig,gravar,nomeVideo,margem_seg_desvioObstaculos + distMax_voo,capacidadeMaxima,P_best,wp_visitados);

tempo_decorrido = toc;

nomeResultado = ["Resultados_2024_tese\VANT_"+num2str(VANT)+"_exp_"+num2str(exp)+".mat"];
save(nomeResultado)
...
RSalvar a figura como um arquivo .fig
saveas(gcf, ["Resultados_2024_tese\melhores\VANT_"+num2str(Nv)+".fig"]);
saveas(gcf, ["Resultados_2024_tese\melhores\VANT_"+num2str(Nv)+".png"]);
