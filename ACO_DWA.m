...
Nv = 1; % Número de veículos==formigas
...
clc;
for i=1:Nv
    rota_vant = galpaoIncicial; % A rota do vant recebe iniciallmente a posição do galpão inicial
  ...
  %Chamando o DWA    
  numfig=3;
  DynamicWindowApproachSample(rota_vant,Obstaculos,1,"Resultado ACO+A*+DWA",numfig)
...
clc;
for i=1:Nv    
  rota_vant = allPoints(P_best{i},:);
  
  %Chamando o DWA    
  numfig=4;
  DynamicWindowApproachSample(rota_vant,Obstaculos,1,"Resultado ACO+DWA",numfig)
