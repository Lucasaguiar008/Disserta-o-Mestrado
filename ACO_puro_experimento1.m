...
% Algoritmo ACO puro para o experimento 1
...
for Nv = 2:5         
    drawnow;
    for simu=1:200        
        tic; % Iniciar o temporizador
        nome_arq = "Resultados_2024_tese/resultados_experimento1/simu_"+simu+"_Nv_"+Nv+".mat";
...
allPoints = [galpaoIncicial;WayPoints;galpaoFinal]; % Definindo o galpão inicial igual ao final
x = allPoints(:,1);
y = allPoints(:,2);
...
drawnow
titulo = ["Resultado MOACS - "+Nv+" VANTs disponíveis"];
title(titulo)
xlabel("Posição eixo x")
ylabel("Posição eixo y")
numfig = 1;
plotando_caminhos_aco_puro(P_best,Nv,x,y,numfig)
savefig(["Resultados_2024_tese\resultados_experimento1\"+titulo])
